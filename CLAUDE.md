...
## 3. Design system
Read ~/.claude/skills/design-system/SKILL.md before any UI work. Defaults: shadcn/Tailwind/Lucide/Inter, restrained neutral palette with one accent, real type scale. Brand overrides come from the project (SHFT: white canvas, lime #A6FF00 accent, dark #1A1A1A, square edges, zero border-radius). For any SHFT site work, ALSO read `website/design.md` first: it is the authoritative brand visual system (exact tokens, type roles, component patterns, motion, guardrails) extracted from the live code, so pages stay visually identical instead of drifting.
